...
class index extends Component {
    ...
    async componentDidMount() {
        ...
        data_filter.date = date_now;
        this.setState({
            data_filter: data_filter,
            date_select: date_now,
        })
        await this.get_list_schedule(this.state.data_filter);
    }
    get_date_now = () => {
    ...
    render() {
        const columns = [
            ...
            },
            {
                ...
                render: (_, item) => (
                    <Space size="middle" >
                        <Tooltip title="Detail"><a onClick={() => this.open_modal('detail', true, item.id)}><AiFillEye /></a></Tooltip>
                        <Tooltip title="Edit"><a disabled onClick={() => this.open_modal('edit', true, item.id)}><AiFillEdit /></a></Tooltip>
        ...
        return (
            <>
                <div className="m-[10px] p-[10px] border shadow-md bg-white">
                    <Button disabled onClick={() => this.open_modal("create", true)}
