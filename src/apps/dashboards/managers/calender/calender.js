...
import {
    Calendar, ConfigProvider, Modal, Divider, Tag, Table, Space, Tooltip, Popconfirm, Button, Avatar, Image, Select
} from 'antd';
...
import Modal_detail from './modal/modal_detail';
import Modal_create from './modal/modal_create';
                        ...
                        <div className='flex items-center '>
                            <Avatar size={{ xs: 20, sm: 20, md: 30, lg: 30, xl: 30, xxl: 30 }} src={item.user_id && item.user_id.avatar} />
                            {
                                (this.format_date(new Date(item.time_localtion_id.show_date))) > this.format_date(DateNow) ?
                                    (
                                        <Tag className="bg-yellow-500 text-white hidden lg:block truncate max-w-[60px]" severity="success" >
                    ...
                    <li className='truncate text-white' key={item.id}>
                        {
                            <div className='flex items-center '>
                                <Avatar size={30} src={item.user_id && item.user_id.avatar} />
                                {
                                    (this.format_date(new Date(item.time_localtion_id.show_date))) > this.format_date(DateNow) ?
                                        (
                                            <Tag className="bg-yellow-500 text-white hidden lg:block max-w-[60px]" severity="success" >
                                                {item.user_id && item.user_id.fullname}
                                            </Tag>
                                        ) :
                                        (this.format_date(new Date(item.time_localtion_id.show_date))) < this.format_date(DateNow) ?
        ...
        return (
            <>
                    ...
                    <ConfigProvider
                        ...
                        <div className='px-[10px]'>
                            <Select
                                ...
                                style={{ width: 150, }}
                                onChange={(value) => this.handleOnchangeUser(value)}
                ...
                <Modal title={`SCHEDULE FOR DATE: ${this.state.date_select}`} open={this.state.modal_list_schedule}
                    ...
                    onCancel={() => this.openForm("modal_list_schedule", false)}
                    width={800}>
                    <div className="m-[10px] p-[10px] border shadow-md bg-white">
